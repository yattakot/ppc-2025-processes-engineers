 ...
 protected:
  void SetUp() override {
    ...
    if (!std::getline(file, line1)) {
      throw std::runtime_error("Failed reading first line");
    }
    if (!std::getline(file, line2)) {
      throw std::runtime_error("Failed reading second line");
    }
...
const std::array<TestType, 5> kTestParam = {
    std::make_pair("test_empty_first.txt", 3), std::make_pair("test_empty_second.txt", 3),
    std::make_pair("test_identical.txt", 0), std::make_pair("test_single_diff.txt", 1),
    std::make_pair("test_diff_length.txt", 2)};

// Генерация списка задач для MPI и SEQ
const auto kTestTasksList = std::tuple_cat(
    ppc::util::AddFuncTask<KulikovDiffCountNumberCharMPI, InType>(kTestParam, PPC_SETTINGS_kulikov_d_coun_number_char),
    ppc::util::AddFuncTask<KulikovDiffCountNumberCharSEQ, InType>(kTestParam, PPC_SETTINGS_kulikov_d_coun_number_char));

// Преобразуем для GTest
