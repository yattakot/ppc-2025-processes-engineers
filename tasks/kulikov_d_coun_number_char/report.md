...
3. Причина замедления:
    ...
    Задача имеет крайне низкую вычислительную плотность: операция s1[i] != s2[i] выполняется за несколько тактов CPU, в то время как передача 200 МБ данных через MPI занимает значительно больше времени.
